...
elseif string.lower(command) == "open" then
	...
	local command2 = { "lua", commandFilePath, unpack(args) }
...
elseif string.lower(command) == "start" then
	...
	local startCommand = { "lua", commandFilePath, unpack(args) }
	os.execute(table.concat(startCommand, " "))
elseif string.lower(command) == "stop" then
	commandFilePath = scriptDirectory .. "startStopScript.lua"
	local stopCommand = { "lua", commandFilePath, unpack(args) }
...
elseif string.lower(command) == "updatesite" then
	...
	print(
		"Invalid command. Supported commands: watch, open, drop, setvisible, get, shutdown, restart, logout, goto, start, stop, --help."
	)
	os.exit(1)
end
